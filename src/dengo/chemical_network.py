...
class ChemicalNetwork(object):
    ...
    def add_reaction(self, reaction, auto_add=True):
        ...
        logger.info("Adding reaction: %s", reaction)
    ...
    def add_cooling(self, cooling_term, auto_add=True):
        ...
        if auto_add:
            ...
            for s in cooling_term.species:
                if s not in self.required_species:
                    logger.error("Species not found: %s", s)
                    raise RuntimeError
    ...
    def print_cooling(self, assign_to):
        ...
        for term in self.cooling_actions:
            # H2 formation/destruction heating is direct chemical energy
            # deposition (4.48 eV per molecule), not a radiative process, so
            # it is not subject to the CIE continuum optical-depth
            # suppression that applies to the radiative cooling terms below.
            # (This used to only exclude the exact key "h2formation", missing
            # "h2formation_extra" — the same physics applies to both.)
            if "h2formation" not in term and "cie_cooling" in self.cooling_actions:
                eq += (
    ...
    def interpolate_species_gamma(self, sp, deriv=False):
        if (sp.name == "H2_1") or (sp.name == "H2_2"):
            ...
            raise RuntimeError(
                "No gamma(T) interpolation function is available for %s" % sp.name
            )
    ...
    def _write_solver_tables_bin(self, output_dir, solver_name):
        """Write rate/cooling/gamma tables as one flat binary file of
        concatenated float64 arrays, in exactly the order that the
        generated ``{solver_name}_solver.C``'s ``_read_tables`` function
        expects (reactions sorted by name, then cooling actions sorted by
        name with their sub-tables sorted, then gamma-interpolated species
        sorted by name -- gamma then dgamma/dT). This has no dependency on
        HDF5: the generated solver reads it back with a plain ``fread``.
        """
        ofn = os.path.join(output_dir, "%s_tables.bin" % solver_name)
        with open(ofn, "wb") as f:
            for name, rxn in sorted(self.reactions.items()):
                rxn.coeff_fn(self).astype("float64").tofile(f)
            for name, action in sorted(self.cooling_actions.items()):
                for tab in sorted(action.tables):
                    action.tables[tab](self).astype("float64").tofile(f)
            for sp in sorted(self.interpolate_gamma_species):
                self.interpolate_species_gamma(sp).astype("float64").tofile(f)
                self.interpolate_species_gamma(sp, deriv=True).astype(
                    "float64"
                ).tofile(f)
    ...
    def write_cython_solver(self, solver_name, output_dir="."):
        ...
        Writes ``{solver_name}_solver.h``, ``{solver_name}_solver.C``,
        ``{solver_name}_solver_run.pyx``, ``{solver_name}_tables.bin``, and
        a copy of the vendored ``BE_chem_solve.C`` integrator into
        ``output_dir``. No HDF5 or SUNDIALS is required to build or run
        the result -- just a C++ compiler and Cython. Call
        ``dengo.solver_build.build_solver(output_dir, solver_name)`` next
        to compile it into an importable module exposing
        ``run_{solver_name}(initial_conditions, dtf, ...)``.
        """
        self.update_ode_species()
        if not os.path.isdir(output_dir):
            os.makedirs(output_dir)
